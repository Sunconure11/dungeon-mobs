...
public class EntityAIFindItem extends EntityAIBase {
	...
	public boolean shouldExecute() {
		List allThings = taskOwner.world.getEntitiesWithinAABB(Entity.class,
				taskOwner.getEntityBoundingBox().expand((double) targetDistance,
						4.0D, (double) targetDistance));
		...
		while (moarLiving.hasNext() || moarItems.hasNext()) {
			...
			if (!gottaCompare) {
				if (moarLiving.hasNext()) {
					...
					System.out.println("[DUNGEON MOBS] ERROR: RM-AI-01 "
							+ "- please report this error code to JaredBGreat.");
				}
			} else {
	...
	public void updateTask() {
		if (livingFlag) {
			...
			if (!hasStuff && !(targetEntity instanceof EntityIronGolem))
				resetTask();
		}
		super.updateTask();
	}
