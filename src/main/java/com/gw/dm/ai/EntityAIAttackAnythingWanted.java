...
public class EntityAIAttackAnythingWanted extends EntityAIAttackMelee {
	...
	public boolean shouldExecute() {
		...
		if (entity == null) return false;
			...
			entityTarget = entity;
			entityPathEntity = attacker.getNavigator().getPathToXYZ(entity.posX, entity.getEntityBoundingBox().minY, entity.posZ);
	...
	public void updateTask() {
		...
		if ((field_75437_f || attacker.getEntitySenses().canSee(entityTarget)) && --field_75445_i <= 0) {
			...
			attacker.getNavigator().tryMoveToXYZ(entityTarget.posX, entityTarget.getEntityBoundingBox().minY, entityTarget.posZ, field_75440_e);
		}
		
		attackTick = Math.max(attackTick - 1, 0);
		double var1 = (double) (attacker.width * 2.0F * attacker.width * 2.0F);
		
		if (attacker.getDistanceSq(entityTarget.posX, entityTarget.getEntityBoundingBox().minY, entityTarget.posZ) <= var1) {
			if (attackTick <= 0) {
				attackTick = 20;
				
				if (attacker.getHeldItemMainhand() != null) attacker.swingArm(EnumHand.MAIN_HAND);
