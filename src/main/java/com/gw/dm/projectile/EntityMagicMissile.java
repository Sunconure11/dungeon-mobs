		...
		if (!world.isRemote) {
			if (var1.entityHit != null && (var1.entityHit instanceof EntityLivingBase)) {
				...
				if (foo.isEntityUndead()) {
					foo.addPotionEffect(new PotionEffect(Potion.getPotionFromResourceLocation("instant_health"), 1, potionLevel));
				}
				else {
					foo.addPotionEffect(new PotionEffect(Potion.getPotionFromResourceLocation("instant_damage"), 1, potionLevel));
				}
			}
		}
		setDead();
