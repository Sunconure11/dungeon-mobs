...
public class EntitySonicBoom extends Entity {
	...
	public EntitySonicBoom(World par1World, EntityLiving par2EntityLiving, double ax, double ay, double az) {
		...
		setSize(1.0F, 1.0F);
		setLocationAndAngles(par2EntityLiving.posX, par2EntityLiving.posY + 1, par2EntityLiving.posZ, par2EntityLiving.rotationYaw, par2EntityLiving.rotationPitch);
	...
	public void onUpdate() {
		...
		if (ticksAlive % 50 == 0) onImpact(false);

		if (!world.isRemote && (shootingEntity != null && shootingEntity.isDead || !world.isAirBlock(new BlockPos(posX, posY, posZ)))) {
			setDead();
		}
		else if (!isDead) {
			super.onUpdate();
			...
			Entity var4 = null;
			List var5 = world.getEntitiesWithinAABBExcludingEntity(this, getEntityBoundingBox().offset(motionX, motionY, motionZ).expand(1.0D, 1.0D, 1.0D));
			double var6 = 0.0D;

			for (int var8 = 0; var8 < var5.size(); ++var8) {
				Entity var9 = (Entity) var5.get(var8);

				if (var9.canBeCollidedWith() && !var9.isEntityEqual(shootingEntity) && !(var9 instanceof EntityDestrachan) && !(var9 instanceof EntityArrow) && !(var9 instanceof EntityThrowable) && !(var9 instanceof EntityEyeRay)) {
					float var10 = 0.3F;
			...
			for (rotationPitch = (float) (Math.atan2((double) var16, motionY) * 180.0D / Math.PI) - 90.0F; rotationPitch - prevRotationPitch < -180.0F; prevRotationPitch -= 360.0F) {
			}
