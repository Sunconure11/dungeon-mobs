...
public class EntityLightball extends EntityThrowable {
	...
	protected void onImpact(RayTraceResult result) {
		if (!world.isRemote) {
			if (result.entityHit instanceof EntityLivingBase) {
				...
				if (damageable(entity)) {
					...
					if (entity.getHealth() <= 0.0F) {
						entity.onDeath(DungeonMobsDamageSource.LIGHT_BALL.causeMobDamage(getThrower()));
					}
				}
			...
			world.playSound((EntityPlayer) null, this.posX, this.posY, this.posZ, SoundEvents.ENTITY_GENERIC_EXPLODE, SoundCategory.BLOCKS, 4.0F, (1.0F + (this.world.rand.nextFloat() - world.rand.nextFloat()) * 0.2F) * 0.7F);
			setDead();
		}
