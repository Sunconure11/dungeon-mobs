...
public class DMGenerationHandler implements IWorldGenerator {
	...
	private void generateTraps(IChunkGenerator iChunkGenerator, World world, Random rand, int chunkX, int chunkZ) {
		...
		BlockPos foo = iChunkGenerator.getNearestStructurePos(world, "Stronghold", new BlockPos(posX, 64, posZ), false);
		...
		if (diff != EnumDifficulty.PEACEFUL) {
			...
			if (diff == EnumDifficulty.NORMAL) max += rand.nextInt(3) + 3;
		...
		for (int i = 0; i < max; i++) {
			...
			if (((i % 3) == match) || (sh && ((i % 2) == 0))) {
				do {
					if (targets.contains(world.getBlockState(new BlockPos(x, lower, z)).getBlock()) && world.isAirBlock(new BlockPos(x, lower + 1, z))) {
						world.setBlockState(new BlockPos(x, lower + 1, z), MiscRegistrar.blockBladeTrap.getDefaultState(), 3);
						break;
					}
					lower++;
				}
				...
				if (sh) {
					do {
						if (targets.contains(world.getBlockState(new BlockPos(x, upper, z)).getBlock()) && world.isAirBlock(new BlockPos(x, upper + 1, z))) {
							world.setBlockState(new BlockPos(x, upper + 1, z), MiscRegistrar.blockBladeTrap.getDefaultState(), 3);
							break;
						}
						upper--;
					}
