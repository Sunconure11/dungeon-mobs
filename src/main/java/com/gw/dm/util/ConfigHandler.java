...
public class ConfigHandler {
	...
	public static void init() {
		File file = new File(ConfigHandler.configDir.toString() + File.separator + DungeonMobs.MODID + ".cfg");
		...
		devMode = config.get("DevNDebug", "DevMode", false, "Allow WIP mobs / features to appear; " + "these mobs may not have AI or even textures and done.  \r\n " + "Keep this false (unless you're actually trying to code new mobs).").getBoolean();
		
		// General Configuration
		config.addCustomCategoryComment("General", "General setting effecting all mobs");
		spawnNaturally = config.get("General", "SpawnNaturally", true, "Determines if dungeon mobs can spawn naturally in the world (\"dark spawn\")").getBoolean();
		addToVanillaDungeons = config.get("General", "AddToVanillaDungeons", true, "Determines dungeons mobs will be added to spawners in vanilla dungeons").getBoolean();
		addToDoomlikeDungeons = config.get("General", "AddToDoomlikeDungeons", true, "Determines dungeons mobs will be added to spawners in doomlike dungeons").getBoolean() && (net.minecraftforge.fml.common.Loader.isModLoaded("DLDungeonsJBG")
				// Yes, I botch some of this and Doomlike Dungeons and have to
				// accomodate for it here... :(
				|| net.minecraftforge.fml.common.Loader.isModLoaded("dldungeonsjbg") || net.minecraftforge.fml.common.Loader.isModLoaded("dldungeonjbg") || net.minecraftforge.fml.common.Loader.isModLoaded("DLDungeonJBG"));
		...
		// Rust Monster Foods
		config.addCustomCategoryComment("Rust Monster Foods", "Items eaten / wanted by " + "rust monsters)");
		replaceRMFoods = config.get("Rust Monster Foods", "Replace", false, "If true the food list for rust monster will be replaced, " + "\r\notherwise it will be kept and added to.").getBoolean();
		rustMonFoodsList = new ArrayList<String>();
		String[] array = config.get("Rust Monster Foods", "FoodList", new String[]{}, "Items the rust monsters want to eat (put metal here); \r\n" + "format \"modid:reistry_name\"").getStringList();
		...
		array = config.get("Shrieker Summons", "ShriekerSummons", new String[]{}, "Mobs the that may be spawned when shriekers shriek (will be added to list); \r\n" + "format \"modid:resource_location\"").getStringList();
		...
		config.addCustomCategoryComment("Special Mob Traits", "Some other special characteristics of some mobs");
		loudVescavor = config.get("Special Mob Traits", "LoudVescavor", false, "True: Use old, LOUD vescavor sound\nFalse: Use new, quieter vescavor sound").getBoolean();
		hardcoreVampire = config.get("Special Mob Traits", "HardcoreVampire", false, "If true vampires will drain levels from the player.").getBoolean();
		hardcoreThoqqua = config.get("Special Mob Traits", "HardcoreThoqqua", true, "If true thoqquas will turn stone into actual lava (temporarily).").getBoolean();
		fireClouds = config.get("Special Mob Traits", "Outerthing Fireclouds", true, "If false outerthings will not use the fire cloud attack.").getBoolean();
		
		
		// Power-Up
		config.addCustomCategoryComment("Power Boost", "Incase you don't think these mobs are tough enough for your " + "OP modpack / gear");
		healthx = config.getFloat("Health Multiplier", "Power Boost", 1.0f, 0.1f, 1000f, "Multiply every mob's base health by this");
		damagex = config.getFloat("Damage Multiplier", "Power Boost", 1.0f, 0.1f, 1000f, "Multiply every mob's base damage by this");
		;
		damageplus = config.getFloat("Damage Additive", "Power Boost", 0.0f, 0.0f, 1000f, "Add this to every mob's base damage " + "(usually less extreme than multiplying");
		rakshasaPtLvl = config.getInt("Rakshasa Damage", "Power Boost", 2, 0, 127, "The damage potion level for rakshasa magic missles.");
		beholderPtLvl = config.getInt("Beholder Damage", "Power Boost", 1, 0, 127, "The potion level for eye rays that act as damage potions.");
	...
	public static void mobDat() {
		File file = new File(ConfigHandler.configDir.toString() + File.separator + "spawn_rules.cfg");
		Configuration config = new Configuration(file);
		config.load();
