...
public class EntityPetrified extends EntityLiving {
	...
	public void setStuff(EntityPlayer player) {
		...
		for (int i = 0; i < 40; i++) {
			if (player.inventory.getStackInSlot(i) != null) stuff[i] = player.inventory.getStackInSlot(i);
		}
	...
	public boolean attackEntityFrom(DamageSource src, float amount) {
		if (src.getTrueSource() instanceof EntityPlayer) {
			if (((EntityPlayer) src.getTrueSource()).getHeldItemMainhand().getItem().canHarvestBlock(Blocks.STONE.getDefaultState())) {
	...
	public void knockBack(Entity par1Entity, int par2, double par3, double par5) {/*Do Nothing*/}
}
