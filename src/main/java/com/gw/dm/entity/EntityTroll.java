...
public class EntityTroll extends EntityMob {
	...
	public EntityTroll(World par1World) {
		...
		this.targetTasks.addTask(3, new EntityAINearestAttackableTarget(this, EntityAnimal.class, 0, true, false, null));
	}
	...
	protected void applyEntityAttributes() {
		...
		this.getEntityAttribute(SharedMonsterAttributes.MAX_HEALTH).setBaseValue(20.0D * ConfigHandler.healthx);
		this.getEntityAttribute(SharedMonsterAttributes.MOVEMENT_SPEED).setBaseValue(0.22D);
		this.getEntityAttribute(SharedMonsterAttributes.ATTACK_DAMAGE).setBaseValue(5.0D * ConfigHandler.damagex + ConfigHandler.damageplus);
	...
	private void turnToStone() {
		if (!world.isRemote) {
			...
			int sx = (int) (getEntityBoundingBox().minX + ((getEntityBoundingBox().maxX - getEntityBoundingBox().minX) / 2) - 0.5);
			
			int sz = (int) (getEntityBoundingBox().minZ + ((getEntityBoundingBox().maxZ - getEntityBoundingBox().minZ) / 2) - 0.5);
			
			world.setBlockState(new BlockPos(sx, (int) getEntityBoundingBox().minY, sz), Blocks.STONE.getDefaultState(), 3);
			world.setBlockState(new BlockPos(sx, (int) getEntityBoundingBox().minY + 1, sz), Blocks.STONE.getDefaultState(), 3);
			this.world.spawnParticle(EnumParticleTypes.SMOKE_NORMAL, this.posX, this.posY, this.posZ, 0.0D, 0.0D, 0.0D);
			this.world.spawnParticle(EnumParticleTypes.SMOKE_NORMAL, this.posX, this.posY + 1, this.posZ, 0.0D, 0.0D, 0.0D);
		}
	}
