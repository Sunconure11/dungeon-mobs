...
public class EntityRakshasaImage extends EntityCreature implements IRangedAttackMob {
	...
	@Override
	public void attackEntityWithRangedAttack(EntityLivingBase var1, float lol) {/*Do NOthing*/}
	...
	public void setDead() {
		...
		for (int i = 0; i < 8; i++) {
			...
			world.spawnParticle(EnumParticleTypes.EXPLOSION_LARGE, posX + foo, posY + 1.0D + foo, posZ + foo, 0.0D, 0.0D, 0.0D);
		}
		
		playSound(SoundEvents.ENTITY_GENERIC_EXTINGUISH_FIRE, 1.0F, 1.0F / (getRNG().nextFloat() * 0.4F + 0.8F));
