...
public class EntityBladeTrap extends EntityLiving {
	...
	protected void applyEntityAttributes() {
		...
		getEntityAttribute(SharedMonsterAttributes.MAX_HEALTH).setBaseValue(100.0D * ConfigHandler.healthx);
	...
	public void onUpdate() {
		...
		if (attackTimer > 11) attackTimer = 11;
		...
		if (attackTimer > 10) {
			...
			if (!flag && !(foo == null) && !foo.isEmpty()) {
				...
				while (iter.hasNext()) {
					...
					playSound(AudioHandler.entityBladeTrapBlade, 1.0F, (world.rand.nextFloat() - world.rand.nextFloat()) * 0.2F + 1.0F);
		...
		for (int i = -6; i < 6; i++)
			for (int j = -6; j < 6; j++)
				for (int k = -6; k < 6; k++) {
					...
					if (world.getBlockState(new BlockPos(x + i, foo, z + k)).getBlock() == MiscRegistrar.blockBladeTrap) {
						count++;
					}
				}
	...
	public boolean findNearbyMoss(int x, int y, int z) {
		for (int i = -4; i < 4; i++) {
			for (int j = -4; j < 4; j++) {
				for (int k = -4; k < 4; k++) {
					Block toTest = world.getBlockState(new BlockPos(x + i, y + j, z + k)).getBlock();
