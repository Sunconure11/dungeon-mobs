...
public class EntityAhriman extends EntityDungeonFlying implements IMob, IRangedAttackMob, IBeMagicMob {
	...
	protected void applyEntityAttributes() {
		...
		getEntityAttribute(SharedMonsterAttributes.MAX_HEALTH).setBaseValue(32.0d * ConfigHandler.healthx);
	...
	private PotionEffect makePotionEffect(String id, int duration, int level) {
		...
		return new PotionEffect(Potion.getPotionFromResourceLocation(id), duration, level);
	}
	
	public PotionEffect[] defineEyeRay() {
		if (this.getAttackTarget() instanceof EntityPlayer) {
			EntityPlayer foo = (EntityPlayer) this.getAttackTarget();
			if (foo.capabilities.isCreativeMode) return new PotionEffect[0];
		}
		
		int bar = 0;
		
		if (this.world.getDifficulty() == EnumDifficulty.EASY) bar = 1;
		if (this.world.getDifficulty() == EnumDifficulty.NORMAL) bar = 2;
		if (this.world.getDifficulty() == EnumDifficulty.HARD) bar = 3;
		...
		for (int i = 0; i < num; i++) {
			...
			if (eff == 0) theEffect[i] = makePotionEffect("slowness", 120, 2);
			else if (eff == 1) theEffect[i] = makePotionEffect("mining_fatigue", 160, 2);
			else if (eff == 2) theEffect[i] = makePotionEffect("instant_damage", 1, ConfigHandler.beholderPtLvl);
			else if (eff == 3) theEffect[i] = makePotionEffect("nausea", 280, 1);
			else if (eff == 4) theEffect[i] = makePotionEffect("resistance", 100, -2);
			else if (eff == 5) theEffect[i] = makePotionEffect("blindness", 120, 1);
			else if (eff == 6) theEffect[i] = makePotionEffect("hunger", 400, 3);
			else if (eff == 7) theEffect[i] = makePotionEffect("weakness", 120, 1);
			else if (eff == 8) theEffect[i] = makePotionEffect("poison", 200, 1);
			else if (eff == 9) theEffect[i] = makePotionEffect("wither", 100, 1);
			else theEffect[i] = makePotionEffect("levitation", 160, 1);
			
			if (i > 0) {
				if (theEffect[i - 1].equals(theEffect[i])) i--;
			}
	...
	public boolean attackEntityFrom(DamageSource par1DamageSource, int par2) {
		...
		if (par1DamageSource.getTrueSource() != null && (par1DamageSource.getTrueSource() instanceof EntityLiving && !(par1DamageSource.getTrueSource() instanceof EntityEyeRay))) {
		...
		if (par1DamageSource.getImmediateSource() != null && par1DamageSource.getImmediateSource() instanceof EntityEyeRay) return false;
	...
	public void doAttack() {
		...
		if (target != null && target.getDistanceSq(this) < ADIST2) {
			...
			double var13 = target.getEntityBoundingBox().minY + target.motionY + (double) (target.height / 4.0F) - posY;
			...
			if (this.canEntityBeSeen(target)) {
				...
				if (world.getDifficulty() == EnumDifficulty.EASY) bar = 1;
				if (world.getDifficulty() == EnumDifficulty.NORMAL) bar = 2;
				if (world.getDifficulty() == EnumDifficulty.HARD) bar = 3;
