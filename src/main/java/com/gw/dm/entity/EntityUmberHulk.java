...
public class EntityUmberHulk extends EntityDungeonMob {
	...
	protected void applyEntityAttributes() {
		...
		getEntityAttribute(SharedMonsterAttributes.MAX_HEALTH).setBaseValue(60.0D * ConfigHandler.healthx);
		getEntityAttribute(SharedMonsterAttributes.MOVEMENT_SPEED).setBaseValue(0.18D);
		getEntityAttribute(SharedMonsterAttributes.ATTACK_DAMAGE).setBaseValue(9.0D * ConfigHandler.damagex + ConfigHandler.damageplus);
		...
		if (confuseTicks % 20 == 0) {
			...
			while (iter.hasNext()) {
				...
				if (canEntityBeSeen(foo) && !foo.capabilities.isCreativeMode) {
					foo.addPotionEffect(new PotionEffect(Potion.getPotionFromResourceLocation("nausea"), 100, 1));
				}
			}
			confuseTicks = 0;
		}
