...
public class EntityManticore extends EntityDungeonMob implements IRangedAttackMob {
	...
	protected void targetArrow(EntityArrow entityarrow) {
		...
		entityarrow.shoot(vx, vy + (dist / 5), vz, 1.6F, (16 - world.getDifficulty().getDifficultyId() * 4));
	}
	...
	protected void applyEntityAttributes() {
		...
		this.getEntityAttribute(SharedMonsterAttributes.MAX_HEALTH).setBaseValue(30.0D * ConfigHandler.healthx);
		this.getEntityAttribute(SharedMonsterAttributes.MOVEMENT_SPEED).setBaseValue(0.3D);
		this.getEntityAttribute(SharedMonsterAttributes.ATTACK_DAMAGE).setBaseValue(3.0D * ConfigHandler.damagex + ConfigHandler.damageplus);
	...
	public boolean attackEntityFrom(DamageSource par1DamageSource, int par2) {
		if ((par1DamageSource.getTrueSource() != null) && (par1DamageSource.getTrueSource().equals(this))) return false;
