...
public class EntityCaveFisher extends EntitySpider {
	...
	protected Entity findPlayerToAttack() {
		...
		if (foo == null) {
			List bar = world.getEntitiesWithinAABB(EntityBat.class, getEntityBoundingBox().expand(16.0F, 8.0F, 16.0F));
		...
		if (stringTimer - foo > 0) {
			if (getAttackTarget() != null && canEntityBeSeen(getAttackTarget())) {
				...
				if (world.isAirBlock(new BlockPos(tarX, tarY, tarZ))) {
					world.setBlockState(new BlockPos(tarX, tarY, tarZ), Blocks.WEB.getDefaultState(), 3);
					
					if (DungeonMobsHelper.getDifficulty(world) > 2) {
						if (world.isAirBlock(new BlockPos(tarX - 1, tarY, tarZ))) {
							world.setBlockState(new BlockPos(tarX - 1, tarY, tarZ), Blocks.WEB.getDefaultState(), 3);
						}
						if (world.isAirBlock(new BlockPos(tarX + 1, tarY, tarZ))) {
							world.setBlockState(new BlockPos(tarX + 1, tarY, tarZ), Blocks.WEB.getDefaultState(), 3);
						}
						if (world.isAirBlock(new BlockPos(tarX, tarY, tarZ - 1))) {
							world.setBlockState(new BlockPos(tarX, tarY, tarZ - 1), Blocks.WEB.getDefaultState(), 3);
						}
						if (world.isAirBlock(new BlockPos(tarX, tarY, tarZ + 1))) {
							world.setBlockState(new BlockPos(tarX, tarY, tarZ + 1), Blocks.WEB.getDefaultState(), 3);
						}
						if (world.isAirBlock(new BlockPos(tarX, tarY + 1, tarZ))) {
							world.setBlockState(new BlockPos(tarX, tarY + 1, tarZ), Blocks.WEB.getDefaultState(), 3);
						}
					}
				}
			}
		...
		if (getAttackTarget() != null && canEntityBeSeen(getAttackTarget())) {
			...
			if (grabTimer >= (foo * 3)) {
				if (bork == 0) {
					...
					double difZ;
					for (difZ = getAttackTarget().posZ - posZ; difX * difX + difZ * difZ < 1.0E-4D; difZ = (Math.random() - Math.random()) * 0.01D) {
						...
						if (!player.capabilities.isCreativeMode) {
							...
							if (!(getAttackTarget() instanceof EntityPlayerMP)) {
								...
								if (playerID == targetID) {
									grabTarget(FMLClientHandler.instance().getClient().player, difX, difZ);
								}
							}
							else {
								DungeonMobsHelper.sendKnockBackPacket((EntityPlayerMP) getAttackTarget(), difX, difZ);
							}
						}
					}
				}
