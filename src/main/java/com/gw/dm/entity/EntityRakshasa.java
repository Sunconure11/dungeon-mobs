...
public class EntityRakshasa extends EntityDungeonMob implements IRangedAttackMob, IBeMagicMob {
	...
	private EntityAIAttackRanged rangeAttack = new EntityAIAttackRanged(this, 0.3F, 5, 40, 24.0F);
	private EntityAIAttackMelee meleeAttack = new EntityAIAttackMelee(this, 0.3F, false);
	...
	protected void applyEntityAttributes() {
		...
		getEntityAttribute(SharedMonsterAttributes.MAX_HEALTH).setBaseValue(42.0D * ConfigHandler.healthx);
	...
	public void attackEntityWithRangedAttack(EntityLivingBase var1, float lol) {
		...
		double var13 = var1.getEntityBoundingBox().minY + var1.motionY + var1.height - getEntityBoundingBox().maxY;
	...
	protected void targetArrow(EntityArrow entityarrow) {
		...
		double vy = target.getEntityBoundingBox().minY + (double) (target.height / 2.0F) + (rand.nextFloat() - 0.5) - entityarrow.posY;
	...
	public void onLivingUpdate() {
		...
		if (castingTimer >= 120 && !world.isRemote) {
			playSound(AudioHandler.entityRakshasael, 1.0F, 1.0F / (getRNG().nextFloat() * 0.4F + 0.8F));
		...
		for (int i = 0; i < imageCount && imageAttempts < 3 + imageCount; i++) {
			...
			if (bar.getCanSpawnHere() && !world.isRemote) {
				if (!world.isRemote) {
					...
					myImages.add(bar);
					if (getAttackTarget() != null) bar.setAttackTarget(getAttackTarget());
				}
				else {
					imageCount--;
					imageAttempts++;
	...
	private void switchPosition() {
		if (!world.isRemote) {
			if (!myImages.isEmpty()) {
				EntityRakshasaImage foo = (EntityRakshasaImage) myImages.get(world.rand.nextInt(myImages.size()));
