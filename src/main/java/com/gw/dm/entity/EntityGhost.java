...
public class EntityGhost extends EntityDungeonMob implements IBeMagicMob {
	...
	protected void damageEntity(DamageSource damageSrc, float damageAmount) {
		if (damageSrc.isMagicDamage() || damageSrc.isCreativePlayer() || (damageSrc == DungeonMobsDamageSource.LIGHT_BALL)) {
			super.damageEntity(damageSrc, damageAmount);
			return;
		}
		if (damageSrc.isFireDamage() || (damageSrc.getTrueSource() instanceof IBeMagicMob)) {
		...
		if (attacker instanceof EntityPlayer) {
			...
			if (equipt.isItemEnchanted()) {
				...
				super.damageEntity(damageSrc, Math.min(damageAmount + rand.nextInt(2), highest));
			}
		}
	}
	...
	public boolean attackEntityFrom(DamageSource source, float amount) {
		...
		if (((target == null) || target.isDead) && (attacker instanceof EntityLivingBase)) {
	...
	protected void applyEntityAttributes() {
		...
		getEntityAttribute(SharedMonsterAttributes.MAX_HEALTH).setBaseValue(35.0d * ConfigHandler.healthx);
	...
	public boolean canHarmGhost(DamageSource damageSrc) {
		Entity attacker = damageSrc.getTrueSource();
		if (damageSrc.isMagicDamage() || damageSrc.isCreativePlayer() || (damageSrc == DungeonMobsDamageSource.LIGHT_BALL) || damageSrc.isFireDamage() || (attacker instanceof IBeMagicMob)) {
			return true;
		}
