...
public class EntityEldermob extends EntityDungeonFlying implements IMob, IRangedAttackMob, IBeMagicMob {
	...
	private static PotionEffect poison = new PotionEffect((Potion) Potion.REGISTRY.getObject(new ResourceLocation("poison")), 300, 0);
	...
	public boolean attackEntityFrom(DamageSource source, float amount) {
		...
		if ((entity != null) && (entity instanceof EntityLivingBase) && !(entity instanceof EntityEldermob)) {
	...
	protected void damageEntity(DamageSource source, float damageAmount) {
		if (source.isFireDamage() || source.isMagicDamage() || (source.getTrueSource() == this)) {
			return;
		}
	...
	protected void applyEntityAttributes() {
		...
		getEntityAttribute(SharedMonsterAttributes.MAX_HEALTH).setBaseValue(66.0d * ConfigHandler.healthx);
	 ...
	 */ protected void dropFewItems(boolean par1, int par2) {
		dropItem(Item.getItemFromBlock(Blocks.SLIME_BLOCK), rand.nextInt(2) + par2);
	...
	public boolean getCanSpawnHere() {
		if (world.provider instanceof WorldProviderEnd && deepEnoughInEnd() && (rand.nextInt(24) == 0)) {
