...
public class EntityShrieker extends EntityMob {
	...
	public boolean attackEntityFrom(DamageSource src, float amount) {
		if (src.getTrueSource() instanceof EntityPlayer) {
			...
			if (!player.capabilities.isCreativeMode) {
				...
				if (rand.nextInt(4 - var) == 0) summonMobs();
			}
		}
	...
	protected void applyEntityAttributes() {
		...
		getEntityAttribute(SharedMonsterAttributes.MAX_HEALTH).setBaseValue(10.0D * ConfigHandler.healthx);
	...
	protected void summonMobs() {
		...
		if (!world.isRemote) {
			for (int i = 0; i < numMobs; i++) {
				...
				EntityLiving attracted = (EntityLiving) EntityList.createEntityByIDFromName(entityNames[fubar], world);
				...
				if (Math.abs(Math.abs(spawnx) - Math.abs(posX)) < 2) {
					if (spawnx < 0) spawnx -= 2.0D;
					else spawnx += 2.0D;
				}
				
				if (Math.abs(Math.abs(spawnz) - Math.abs(posZ)) < 2) {
					if (spawnz < 0) spawnz -= 2.0D;
					else spawnz += 2.0D;
				}
				
				makeSummonIgnoreHeight(attracted);
				
				attracted.setLocationAndAngles(spawnx, spawny + 1, spawnz, MathHelper.wrapDegrees(world.rand.nextFloat() * 360.0F), 0.0F);
