...
public class EntityNetherHound extends EntityDungeonMob {
	...
	public boolean getCanSpawnHere() {
		if (hellHoundIg || (world.provider instanceof WorldProviderHell) || DungeonMobsHelper.isNearSpawner(world, this, mobName)) {
	...
	protected void applyEntityAttributes() {
		...
		getEntityAttribute(SharedMonsterAttributes.MAX_HEALTH).setBaseValue(28.0D * ConfigHandler.healthx);
	...
	public void onLivingUpdate() {
		if (getAttackTarget() != null && canEntityBeSeen(getAttackTarget())) {
			...
			if (fooX + fooY + fooZ <= 16.0D) {
				...
				if (flameTimer >= (170 - (40 * DungeonMobsHelper.getDifficulty(world)))) playSound(AudioHandler.entityNetherHoundFire, 1.0f, 1.0f);
	...
	private void makeFire(int x, int y, int z) {
		...
		if (world.isAirBlock(where)) world.setBlockState(where, Blocks.FIRE.getDefaultState());
