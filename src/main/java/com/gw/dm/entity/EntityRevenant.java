...
public class EntityRevenant extends EntityZombie implements IBeMagicMob {
	...
	protected void applyEntityAttributes() {
		...
		getEntityAttribute(SharedMonsterAttributes.MAX_HEALTH).setBaseValue(HEALTH * ConfigHandler.healthx);
		...
		getEntityAttribute(SharedMonsterAttributes.ATTACK_DAMAGE).setBaseValue(5.0d * ConfigHandler.damagex + ConfigHandler.damageplus);
		...
		if (super.attackEntityAsMob(victim)) {
			if (!isChild() && (victim instanceof EntityLivingBase)) {
				switch (world.getDifficulty()) {
					case EASY:
						((EntityLivingBase) victim).addPotionEffect(new PotionEffect(MobEffects.WITHER, 80, 1));
						break;
					case HARD:
						((EntityLivingBase) victim).addPotionEffect(new PotionEffect(MobEffects.WITHER, 120, 1));
						break;
					case NORMAL:
						((EntityLivingBase) victim).addPotionEffect(new PotionEffect(MobEffects.WITHER, 100, 1));
						break;
					case PEACEFUL:
					default:
						break;
	...
	public boolean getCanSpawnHere() {
		...
		if (world.canBlockSeeSky(new BlockPos(posX, posY, posZ)) || (!world.isRemote && world.isDaytime())) {
			return false;
		}
	...
	private void regenerate() {
		if (!isDead && (getHealth() > 0) && (getHealth() < HEALTH) && !isBurning()) {
